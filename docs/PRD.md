...
| FR-06 | The destination is the source path with only its final extension replaced by `.md`. | Must |
...
- **Correctness:** UTF-8, UTF-8 with BOM, CRLF, repeated VTT cues, cue settings, speaker tags, malformed VTT, empty files, mixed-case extensions, and filename edge cases have automated coverage.
