...
- [ ] 7.1 Commit and push the accepted candidate to `main`; verify the worktree is clean, local `main` matches `origin/main`, and public default-branch metadata declares `0.1.1`
- [ ] 7.2 Confirm tag and GitHub release `0.1.1` do not exist and re-verify `0.1.0` asset names and hashes; stop for review on any conflict or historical mismatch
- [ ] 7.3 Create immutable GitHub release `0.1.1` with exactly `main.js`, `manifest.json`, and `styles.css`, then verify remote hashes match the accepted staging directory and `0.1.0` remains unchanged
